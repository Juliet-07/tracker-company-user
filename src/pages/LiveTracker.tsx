...
import { RefreshCw, Plus, Minus, Crosshair, Expand, Layers, MapPin, Car, Play, Pause, AlertTriangle, Gauge, Route, Download, Settings } from "lucide-react";
...
const LiveTracker = () => {
  ...
  const addVehicleToMap = (vehicle: Vehicle) => {
    ...
    const marker = window.L.marker([vehicle.lat, vehicle.lng], { icon }).addTo(mapInstanceRef.current);
    
    const popupContent = `
      <div style="padding: 12px; min-width: 200px;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
          <h4 style="font-weight: 600; color: #374151; margin: 0;">${vehicle.id}</h4>
          <span style="padding: 4px 8px; font-size: 12px; border-radius: 12px; color: white; background-color: ${getStatusColor(vehicle.status)};">${vehicle.status}</span>
        </div>
    ...
    setVehicles(prevVehicles => 
      prevVehicles.map(vehicle => {
        if (vehicle.status === 'moving') {
          ...
          const newSpeed = Math.max(30, Math.min(80, vehicle.speed + (Math.random() - 0.5) * 10));
          ...
          return { ...vehicle, lat: newLat, lng: newLng, speed: Math.round(newSpeed) };
        }
        return vehicle;
      })
    );
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      if (mapRef.current && window.L && !mapInstanceRef.current) {
        mapInstanceRef.current = window.L.map(mapRef.current).setView([37.7749, -122.4194], 10);
  ...
  const avgSpeed = Math.round(vehicles.filter(v => v.status === 'moving').reduce((sum, v) => sum + v.speed, 0) / movingCount) || 0;
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <h2 className="text-lg font-semibold text-gray-800">Live Tracking Controls</h2>
            ...
            <div className="bg-white rounded-lg shadow-sm h-full overflow-hidden">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-4 text-sm">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-gray-600">Moving ({movingCount})</span>
                    </div>
                    <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-gray-600">Offline ({offlineCount})</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="relative h-full">
                ...
                <div className="absolute top-4 right-4 space-y-2 z-[1000]">
                  <Button variant="outline" size="sm" className="w-10 h-10 p-0 bg-white shadow-md hover:bg-gray-50" onClick={() => mapInstanceRef.current?.zoomIn()}>
                    <Plus className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" className="w-10 h-10 p-0 bg-white shadow-md hover:bg-gray-50" onClick={() => mapInstanceRef.current?.zoomOut()}>
                    <Minus className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" className="w-10 h-10 p-0 bg-white shadow-md hover:bg-gray-50" onClick={() => mapInstanceRef.current?.setView([37.7749, -122.4194], 10)}>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-sm h-[400px]">
              <div className="p-4 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-800">Assigned Vehicles</h3>
                <p className="text-sm text-gray-600">{vehicles.length} vehicles assigned to you</p>
              </div>
                ...
                {vehicles.map((vehicle) => (
                  <div
                    ...
                    className={`p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${
                      selectedVehicle === vehicle.id ? 'bg-blue-50 border-blue-500' : ''
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-gray-800">{vehicle.id}</span>
                      <span className={`px-2 py-1 ${getStatusBgClass(vehicle.status)} text-white text-xs rounded-full capitalize`}>
                        {vehicle.status}
                      </span>
                    </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-4">
              ...
              <div className="space-y-2">
                <Button variant="ghost" className="w-full justify-start bg-gray-50 hover:bg-gray-100">
                  ...
                  <span className="text-sm font-medium">View Route History</span>
                </Button>
                <Button variant="ghost" className="w-full justify-start bg-gray-50 hover:bg-gray-100">
                  <Download className="mr-3 h-4 w-4 text-blue-600" />
                  <span className="text-sm font-medium">Export Tracking Data</span>
                </Button>
                <Button variant="ghost" className="w-full justify-start bg-gray-50 hover:bg-gray-100">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold text-green-600">{movingCount}</p>
              </div>
